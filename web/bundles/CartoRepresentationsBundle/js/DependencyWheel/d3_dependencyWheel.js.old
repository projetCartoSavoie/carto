function D3_DependencyWheelRepresentation(){}
...
D3_DependencyWheelRepresentation.prototype.show = function(data) {

  // data is file path
  if(typeof data === "string"){
    d3.json(data, function(error, root) {
      if (error) alert(error);
      D3_DependencyWheelRepresentation.load(root);
    });
  }
  // data is json
  else {
    D3_DependencyWheelRepresentation.load(data);
  }
}

D3_DependencyWheelRepresentation.load = function(json) {
//d3.chart.dependencyWheel = function(options) {
  ...
  var graph = formatter.to_matrix(json); 
