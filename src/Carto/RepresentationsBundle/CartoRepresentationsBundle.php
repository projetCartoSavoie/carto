<?php
/**
	* Bundle qui gère les représentations de données (graph, tree, radialTree)
	*
	*
	* @author Rémy Cluze <Remy.Cluze@etu.univ-savoie.fr>
	* @author Anthony Di Lisio <Anthony.Di-Lisio@etu.univ-savoie.fr>
	* @author Juliana Leclaire <Juliana.Leclaire@etu.univ-savoie.fr>
	* @author Rémi Mollard <Remi.Mollard@etu.univ-savoie.fr>
	* @author Céline de Roland <Celine.de-Roland@etu.univ-savoie.fr>
	*
	* @version 1.0
	*/
namespace Carto\RepresentationsBundle;

use Symfony\Component\HttpKernel\Bundle\Bundle;

/**
	* Bundle qui gère les représentations de données (graph, tree, radialTree)
	*
	*/
class CartoRepresentationsBundle extends Bundle
{
}
